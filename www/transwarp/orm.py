...
class Field(object):
    ...
    def __init__(self, **kw):
        #print kw.get('name', None), 'ddddddddddddddddddddddddddddddddddddddd'
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.iteritems():
            if isinstance(v, Field):
                ...
                #check deplicate primary key:
                if v.primary_key:
                    if primary_key:
                        raise TypeError('Cannot define more than 1 primary ke in class: %s' % name)
                    if v.updatable:
                        logging.warning('NOTE: change primary ke to noe-updatable')
                        v.updatable = False
                    if v.nullable:
